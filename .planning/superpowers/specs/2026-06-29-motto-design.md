...
- **Output stays portable** even though the tooling is Claude-Code-specific. Built skills are plain folders, so a dist plugin can also be zipped for upload to Claude Desktop / claude.ai.

## Architecture

### Distribution

Motto ships as a **Claude Code plugin**, installed as a per-project dependency and updated in one command; never hand-edited. Its CLI (`lint`, `build`) rides inside the plugin.

Where the framework physically lives is an **open install-mechanism decision** (see Open items): the global plugin cache (standard for CC plugins, but per-project version pinning isn't native) versus a repo-local managed `.motto/` folder (gitignored, not hand-edited — gives per-project pinning and a concrete home for the meta-skill, but is non-standard plugin loading).
...
Motto itself is an installed dependency, not hand-edited source (see Distribution for where it lives).

### Output

`motto build` emits `dist/` as **self-contained Claude Code plugins**. Plugin names are defined in `motto.yaml` (not auto-derived):

- `dist/public/` → public plugin (e.g. `myplugin`) → consumers install it, see `/myplugin:*`. Motto's name appears nowhere.
- `dist/private/` → private plugin (e.g. `myplugin-pro`) → local only, never published. **Optional** — emitted only if the project has private skills and a private plugin name is set.
...
Project **identity** (`name`, `version`, `description`), **plugin names** (public + optional private), and future **preferences** (e.g. output language) live in a project-specific **`motto.yaml`** — the single config file. Values defined later by the author.
...
`audience` is binary. `public` = published *and* available to you locally; `private` = yours only.
...
A template is a **schema add-on**: it layers extra required sections/fields onto base, lives in the framework's `schema/templates/`, and a skill opts in via `template: <name>`. A template may also waive base rules (the Role escape hatch). The linter validates a skill against `base + chosen template`.

Template **file format** — Markdown, YAML, or YAML that references Markdown procedures/tools — is **deferred**, decided when the first concrete template is built.
...
- **`motto build`** — runs lint first (refuses to build on failure), then resolves deps + copies → `dist/public` (and `dist/private` if configured) as plugins, writing each `plugin.json` from `motto.yaml`.
...
- **Install mechanism** — global plugin cache vs repo-local `.motto/`; resolve in the implementation plan.
- **`motto.yaml`** — project identity, plugin names (public + optional private), and preferences (e.g. output language); authored later.
- **Template file format** (md / yaml) — decided with the first concrete template.
- **`--zip` output** for Claude Desktop / claude.ai upload — trivial add, deferred until needed.
