...
All 6 Warning findings fixed by gsd-code-fixer before phase verification; suite 195/195 green and `motto lint` clean after every commit:

| Finding | Outcome | Commit |
|---------|---------|--------|
| WR-01 | fixed — §6 splits template:"" (step 2) vs template:null (step 1, typeof object) | `603d974` |
| WR-02 | fixed — §6 states both section-tag regexes lack an end anchor (symmetric) | `ccfbd66` |
| WR-03 | fixed — §5 documents actual non-array behavior (silently ignored; validator improvement noted as future backlog item) | `ce362b7` |
| WR-04 | fixed — doc-sync test extended with 8 segments incl. src/frontmatter.js; adversarially verified non-vacuous | `dd4fac2` |
| WR-05 | fixed — build-skill guard states substring match; DOC-06 greps still 0 | `84f7262` |
| WR-06 | fixed — README comment: dist/private/ appears only with audience: private skills | `c230bfc` |

Info findings (IN-01..IN-04) acknowledged, not blocking: dist/ is untracked and regenerated at build/publish time.
