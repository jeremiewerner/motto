...
// of static error-message segments is derived from src/schema.js and
// src/lint.js's own source text, then asserted as a substring of
// shared/references/skill-schema.md. Never the reverse — iterating the doc's
// own tables and checking them against the code would invert the
// source-of-truth relationship and mask a doc going stale.
//
...
describe('doc-sync (DOC-06)', () => {
  it('every curated static segment is live source text AND is quoted in skill-schema.md', async () => {
    const [schemaSrc, lintSrc, docText] = await Promise.all([
    ...
    for (const seg of staticSegments) {
      ...
      // catches the case where schema.js/lint.js changed and the curated list
      // here was never updated to match.
      assert.ok(
        schemaSrc.includes(seg) || lintSrc.includes(seg),
        `stale curated segment (not in schema.js/lint.js source): "${seg}"`,
      );
